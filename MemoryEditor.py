# здесь находится класс, который принимает на вход имя процесса 
import pymem # как раз взламывает процессы
import ctypes
...
class MemoryEditor:
    ...
    def search_value(self, value: int, chunk_size = 0x1000 ) -> list:
        """выполняет поиск заданного значения (value) в памяти процесса"""
        search_results = []
    ...
    def is_memory_readable(self, address) -> bool:
        """ проверяет, доступна ли память по указанному адресу для чтения."""
    ...
    def search_next_value(self, addresses: list, next_value: int) -> list:
        """ищет новое значение (next_value) только среди адресов, найденных на предыдущем этапе поиска"""
        search_results = []
    ...
    def replace_value(self, addresses: list, new_value: int) -> None:
        """заменяет значения по указанным адресам (addresses) на новое значение (new_value)."""
